...
class FineUploader
{
	...
	public function getInitialFiles()
	{
		...
		for ($i = 0; $i < 5000; $i++) {
			array_push($initialFiles, ["name" => "name" + $i, uuid => "uuid" + $i, thumbnailUrl => "/test/dev/handlers/vendor/fineuploader/php-traditional-server/fu.png"]);
		}
	...
	public function combineChunks($uploadDirectory, $name = null)
	{
		...
		$targetPath = join(DIRECTORY_SEPARATOR, [$uploadDirectory, $name]);
		...
		if (!is_null($this->sizeLimit) && filesize($targetPath) > $this->sizeLimit) {
			...
			return ["success" => false, "preventRetry" => true];
		}
	...
	public function handleUpload($uploadDirectory, $name = null)
	{
		...
		if ($this->allowedExtensions && !in_array(strtolower($ext), array_map("strtolower", $this->allowedExtensions))) {
			...
			$name = $_REQUEST['qquuid'] . "." . strtolower($ext);
		}
		...
		if ($totalParts > 1) {
			...
			# non-chunked upload
			$target = join(DIRECTORY_SEPARATOR, [$uploadDirectory, $name]);
	...
	public function handleDelete($uploadDirectory, $uuid)
	{
		...
		if (!is_dir($target)) {
			array_map('unlink', glob($uploadDirectory . DIRECTORY_SEPARATOR . $uuid . ".*"));
